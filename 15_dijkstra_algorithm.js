...
const dijkstra = (graph, start, end) => {
  ...
  while (currentNode != end) {
    ...
    // find shortest distance from graph.start node
    let shortestPath = {
      path: '',
      distance: Infinity
    }
    for (let key in graph[currentNode]) {
      if (graph[currentNode][key] < shortestPath.distance) shortestPath = {
        path: key,
        distance: graph[currentNode][key]
      }
    }
    // add to result.distance
    result.distance += shortestPath.distance
    // set new start node
    currentNode = shortestPath.path
    // push current node to result.path
